...
class Auth{
    ...
    protected function getAuthList($uid,$type) {
        ...
        //读取用户组所有权限规则
        $rules = \think\Db::name($this->_config['auth_rule'])->where($map)->field('title,condition,name')->select();
        ...
        foreach ($rules as $rule) {
            if (!empty($rule['condition'])) { //根据condition进行验证
                ...
                if ($condition) {
                    $authList[] = strtolower($rule['title']);
                }
            } else {
                //只要存在就记录
                $authList[] = strtolower($rule['title']);
    ...
    protected function getUserInfo($uid) {
        ...
        if(!isset($userinfo[$uid])){
            $userinfo[$uid]=\think\Db::name($this->_config['auth_user'])->where(array('admin_id'=>$uid))->find();
        }
        return $userinfo[$uid];
    }
